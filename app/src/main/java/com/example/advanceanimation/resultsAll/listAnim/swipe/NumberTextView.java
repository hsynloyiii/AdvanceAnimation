...
//        textPaint.setTextSize(AndroidUtilities.dp(size));
        textPaint.setTextSize((float) Math.ceil(1F * size));
        oldLetters.clear();
        letters.clear();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float translationHeight = addNumber ? (float) Math.ceil(1 * 4) : height;
        ...
        for (int a = 0; a < count; a++) {
            ...
            canvas.restore();
            canvas.translate(layout != null ? layout.getLineWidth(0) : old.getLineWidth(0) + (float) Math.ceil(1 * 1), 0);
